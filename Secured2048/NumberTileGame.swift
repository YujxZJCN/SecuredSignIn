...
class NumberTileGameViewController : UIViewController, GameModelProtocol {
    ...
    let thinPadding: CGFloat = 4.0
    let thickPadding: CGFloat = 8.0
...
//        self.view.backgroundColor = #colorLiteral(red: 0.07450980392, green: 0.1019607843, blue: 0.1843137255, alpha: 1)
//    }
    ...
    func followUp() {
        ...
        if m.userHasLost() {
            ...
            NSLog("You lost...")
//            let para = ["score" : m.score]
//            Alamofire.request("https://ckcsu.com/admin/2048", method: .post, parameters: para).responseData { (response) in
//                if let html = response.result.value, let doc = try? HTML(html: html, encoding: .utf8) {
//                    if let highestScore = doc.content {
//                        let alert = UIAlertController(title: "You lost...", message: "Your Highest Score is \(highestScore).\n\nData has been uploaded to CKCSU Server.", preferredStyle: .alert)
